...
public class TimeUtils {
    ...
    public static String timeToArrival(LocalTime arrivalTime) {
        long minutes = getToArrivalMinutes(arrivalTime);
        long hours = getToArrivalHours(arrivalTime);
        return "Через " + ((hours != 0) ? hours + " ч. " : "") +  minutes + " мин.";
    }

    public static long getToArrivalMinutes(LocalTime arrivalTime) {
        LocalTime currentTime = LocalTime.now();
        Duration remainingTime = Duration.between(currentTime, arrivalTime);
        return remainingTime.toMinutes() % 60;
    }

    public static long getToArrivalHours(LocalTime arrivalTime) {
        LocalTime currentTime = LocalTime.now();
        Duration remainingTime = Duration.between(currentTime, arrivalTime);
